...
pub fn set_ohos_app(app: openharmony_ability::OpenHarmonyApp) {
  if let Err(e) = openharmony_ability_plugin_webview::set_cookie_sync_app(&app) {
    log::error!("[wry] failed to set up the webview-cookie sync bridge: {}", e);
  }
}
...
/// thread requirements.
///
/// OHOS callers today: none in-tree. tauri-runtime-wry's
/// `webview_runtime_installed` probe deliberately hardcodes `true` on OHOS
/// (ArkWeb is an always-present system component, and on systems below API 20
/// this query degrades to `Err` BY DESIGN — wiring the probe to
/// `webview_version().is_ok()` would fail-closed every `create_webview` on
/// those devices), and tauri's `webview_version` re-export chain is
/// cfg-excluded on OHOS. This function exists as correct public API surface
/// for direct wry users on OHOS.
///
